## Filename: first_factor.py
 # Author: Marcelo Feitoza Parisi
 # 
 # Description: First Factor Auth
...
def authenticate(this_request):
    ...
    # Build endpoint URL
    endpoint = authelia_config["proto"] + "://" + authelia_config["host"] + ":" + authelia_config["port"] + authelia_config["first"]

    # Empty cookie
    my_cookie = ""

    # Perform a POST request to our endpoint using application/json mime
    req = request.Request(endpoint, method="POST")
    req.add_header('Content-Type', 'application/json')
    my_request = json.dumps(this_request)
